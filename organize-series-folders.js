...
// Step 2: For each series, create subfolder, move md, copy original files
seriesDefs.forEach(({ id, test }) => {
  const mdPath = path.join(seriesDir, `${id}.md`);
  if (!fs.existsSync(mdPath)) {
    console.log('Skipping (no md):', id);
    return;
  }
  
  const targetDir = path.join(seriesDir, id);
  fs.mkdirSync(targetDir, { recursive: true });
  
  // Move md file into subfolder
  const newMdPath = path.join(targetDir, `${id}.md`);
  fs.renameSync(mdPath, newMdPath);
  ...
  matched.forEach(p => {
    ...
    try {
      fs.cpSync(p.postPath, destDir, { recursive: true, force: true });
    } catch (e) {
